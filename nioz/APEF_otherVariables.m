...
for j = 1:length(windowTime)
    ...
    RaVec = [RaVec g./H/rho0.*(EP1-EP2).*H^3./nu_cst./kappa_T];
    ...
    % other variables needed for viscosity
    rhoVec = [rhoVec rho0];
    tempVec = [tempVec nanmean(T1(:,j))];
end
disp('  -> done!')
...
tau_N = 1./sqrt(N2Vec);
nu = visc35(tempVec);
tau_O = sqrt(epsVec./nu);
Reb = tau_N./tau_O;



%% Few plots.
APEF_otherVariables_plot
