...
a3 = axes('position',[0.05 0.59 0.25 0.25]) ; % inset
                                              %m_proj('mercator', 'long',[lon_min lon_max],'lat',[lat_min lat_max]);
HH = contour3(longitude,latitude,abs(bathy),100, 'k');
...
for i = 1:7%length(latVec)
    ...
    plot3(lonVec(i), latVec(i), abs(bathy(I)),'marker', '.', 'color', 'm','markersize',14);
end
...
for i = 1:length(latVec2)
    ...
    plot3(lonVec2(i), latVec2(i), abs(bathy(I)),'marker', '.', 'color', 'g','markersize',14);
end
...
plot3(ln, lt, zn, 'color','m','linewi',1,'linestyle', '--')
...
plot3(ln, lt, zn, 'color','g','linewi',1,'linestyle', '--')

% $$$ plot3([-16.1120, -15.3720],[55.820, 55.023],'color','m','linewi',1,'linestyle', '--'); 
% $$$ plot3([-15.969, -15.126],[55.820, 55.023],'color','g','linewi',1,'linestyle', '--'); 
...
ti = ylabel(c,'u (cm s^{-1})', 'FontSize', FS, 'fontweight', 'bold','color',[1 1 1]*0);
...
print('-depsc', 'RockallMapAviso_multiBeam.eps')
