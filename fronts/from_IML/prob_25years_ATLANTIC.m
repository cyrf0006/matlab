function [mean_prob_25years,cumul_prob,used_files300]=prob_25years_ATLANTIC()
...
% l'hiver, moyennée sur 25 ans (1986-2010)
...
mean_prob_25years=zeros(dim_im(1),dim_im(2)); % création de la matrice de probabilités de détection de fronts pour la saison

extension  = 'mat';
used_files300=0; %compteurs de fichiers utilisés; pour validation doit être = à 75 (3 mois * 25 ans) 
...
%                              - que l'extension est .mat
               ...
               used_files300=used_files300+1;
               for j=1:dim_im(1)
                    for k=1:dim_im(2)
                        cumul_prob(j,k)=cumul_prob(j,k)+input(j,k);
                        mean_prob_25years(j,k) = (cumul_prob(j,k)./used_files300);       
                    end
               end
            end                     
        end    
