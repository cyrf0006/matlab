...
%   Ce programme va chercher les matrices de probabilités mensuelles de 
%   détection de fronts et va calculer les climatologies selon la saison 
%   choisie Les matrices sources se trouvent sous:
%   J:\matlab_atlantic\moyenne_INPUT\
%   Les résulats sont enregistrés sous 
%   J:\matlab_atlantic\probabilite_OUTPUT\saisonniere
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   

% IMPORTANT: si les saisons sont divisées telles que par Environnement Canada
% les lignes 27-36 et 67-75 doivent être mises en commentaires alors que
% 39-49 et 77-85 doivent être décommentées,
...
% COMMENTER pour SAISONS SELON ENVIRONNEMENT CANADA
if (season==1)        
       [mean_prob_season,cumul_prob,used_files75]=prob_winter_25years_ATLANTIC();
elseif (season==2)
       [mean_prob_season,cumul_prob,used_files75]=prob_spring_25years_ATLANTIC();
elseif (season==3)
       [mean_prob_season,cumul_prob,used_files75]=prob_summer_25years_ATLANTIC();
elseif (season==4)
       [mean_prob_season,cumul_prob,used_files75]=prob_autumn_25years_ATLANTIC();
end

% DÉCOMMENTER pour SAISONS SELON ENVIRONNEMENT CANADA
% if (season==1)        
%        [mean_prob_season,cumul_prob,used_files75]=ECprob_winter_25years_ATLANTIC();
% elseif (season==2)
%        [mean_prob_season,cumul_prob,used_files75]=ECprob_spring_25years_ATLANTIC();
% elseif (season==3)
%        [mean_prob_season,cumul_prob,used_files75]=ECprob_summer_25years_ATLANTIC();
% elseif (season==4)
%        [mean_prob_season,cumul_prob,used_files75]=ECprob_autumn_25years_ATLANTIC();
% end
...
%     imagesc(lon(1,:)',lat(:,1),year_probability)
%     imagesc(lon(1,:)',lat(:,1),cloud_season)
    ...
    title([ strseason ' front detection probability (%) averaged over 1986-2010'  ]) 
                                                                      
    save([dest 'Prob_' strseason '_1986-2010.mat'],'lat','lon','used_files75','mean_prob_season','cumul_prob')
    saveas(h,[dest 'Prob_'  strseason '_1986-2010'],'png') 
    saveas(h,[dest 'Prob_' strseason '_1986-2010'],'fig')
    
    clear ('mean_prob_season','cumul_prob');
    delete(h)
