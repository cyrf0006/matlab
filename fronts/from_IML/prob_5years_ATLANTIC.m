function [mean_prob_5years,cumul_prob,used_files60]=prob_5years_ATLANTIC()
...
mean_prob_5years=zeros(dim_im(1),dim_im(2)); % création de la matrice de probabilités de détection de fronts pour la saison

extension  = 'mat';
used_files60=0; %compteurs de fichiers utilisés; pour validation doit être = à 60 (12 mois * 5 ans) 
...
%                              - que l'extension est .mat
               ...
               used_files60=used_files60+1;
               for j=1:dim_im(1)
                    for k=1:dim_im(2)
                        cumul_prob(j,k)=cumul_prob(j,k)+input(j,k);
                        mean_prob_5years(j,k) = (cumul_prob(j,k)./used_files60);       
                    end
               end
            end                     
        end    
