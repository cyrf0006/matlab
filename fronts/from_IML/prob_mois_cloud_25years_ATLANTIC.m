...
%
% Ce script créer la matrice de fréquence de nuage pour
% chacun des mois, moyennée sur 25 ans (1986-2010)

function [monthly_cloud_prob_25years,cumul_prob,used_files25] = prob_mois_cloud_25years_ATLANTIC(strmonth)
...
cumul_prob = zeros(dim_im(1),dim_im(2));  % création de la matrice qui cumule les probabilités de couverture nuageuse de tous les mois, toutes les valeurs sont = 0
monthly_cloud_prob_25years=zeros(dim_im(1),dim_im(2)); % création de la matrice de probabilités de couverture nuageuse pour les 25 ans
...
%                               - l'extension est .mat
               ...
               input=data.cloud_prob;
               used_files25=used_files25+1;
               for j=1:dim_im(1)
                    for k=1:dim_im(2)
                        cumul_prob(j,k)=cumul_prob(j,k)+input(j,k);
                        monthly_cloud_prob_25years(j,k) = (cumul_prob(j,k)./used_files25);  
                    end
               end
            end                    
    end    
